...
LOO = function(xl,class) {
  ...
  label = paste("   K = ", min, "\n", "   LOO = ", lOOmin, sep = "")
  text( lOOmin, labels = label, pos=4, col = "red")
  map(min)
}

map = function(k){
  text <- paste("Карта классификации KWNN для k = ", k)
  plot(iris[, 3:4],main=text,xlab = "Длина Листа", ylab = "Ширина Листа", pch = 21, bg = colors[xl$Species], col = colors[xl$Species],asp='1')
